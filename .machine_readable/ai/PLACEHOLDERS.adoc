...
sed -i 's/{{project}}/my_project/g' $(grep -rl '{{project}}' .)
sed -i 's/pseudoscript/my-project/g' $(grep -rl 'pseudoscript' .)
...
| `pseudoscript` | Repository name (slug) | `my-project` | CONTRIBUTING.md, SECURITY.md, CODE_OF_CONDUCT.md, cliff.toml |
