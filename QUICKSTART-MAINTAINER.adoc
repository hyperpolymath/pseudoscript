...
// Replace rsr-template-repo, pseudoscript, {{DEPS}} with actuals
...
| `$PREFIX/share/pseudoscript/`
| Data files, assets

| `$PREFIX/share/doc/pseudoscript/`
| Documentation
...
Default config location: `$XDG_CONFIG_HOME/pseudoscript/config.toml`

Fallback: `$HOME/.config/pseudoscript/config.toml`
...
Or via OPSM: `opsm update pseudoscript`
...
[source,bash]
----
just install --prefix=/opt/pseudoscript-instance1 --config=/etc/pseudoscript/instance1.toml
just install --prefix=/opt/pseudoscript-instance2 --config=/etc/pseudoscript/instance2.toml
----
